...
```bash
/absolute/path/to/AgentMicro/scripts/build-macos.sh
/absolute/path/to/AgentMicro/scripts/package-dmg.sh
```
...
`~/Library/Application Support/AgentMicro/Backups/`. A complete backup can be
...
```bash
MACOS_SIGN_IDENTITY="Developer ID Application: Example Name (TEAMID)" \
MACOS_NOTARY_PROFILE="agent-micro-notary" \
scripts/package-dmg.sh
```
...
```bash
swift macos/AgentMicro/Support/IconSource/generate-icon.swift \
  macos/AgentMicro/Support/Assets.xcassets/AppIcon.appiconset
```
