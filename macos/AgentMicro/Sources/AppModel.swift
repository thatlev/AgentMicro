...
final class AppModel: ObservableObject {
    ...
    private enum DefaultsKey {
        static let completedOnboarding = "AgentMicro.completedOnboarding.v1"
        static let requestedLaunchAtLogin = "AgentMicro.requestedLaunchAtLogin.v1"
        static let lastUnpatchedNotification = "AgentMicro.lastUnpatchedNotification.v1"
    }

    init(
        legacyMigration: LegacyMigrationResult = LegacyMigrationResult(),
        bridgeEngine: AgentMicroBridgeEngine = AgentMicroBridgeEngine(),
    ...
    private func notifyIfNewUnpatchedBuild() {
        ...
        let request = UNNotificationRequest(
            identifier: "agent-micro-chatgpt-\(identity.hashValue)",
            content: content,
            trigger: nil
        )
