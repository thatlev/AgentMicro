...
//
//   default      BLE central linked to the AgentMicroRemote iPhone app.
...
//
//   --emulate    Built-in virtual AgentMicro: answers the RPC surface
...
//
// Settings sync (BLE mode only): ChatGPT stores the AgentMicro layout and
...
//
// Build:  swiftc -O tools/AgentMicroBridge/main.swift tools/AgentMicroBridge/T3Backend.swift -o tools/AgentMicroBridge/codexbridge
// Run:    ./tools/AgentMicroBridge/codexbridge            # bridge the iPhone app
//         ./tools/AgentMicroBridge/codexbridge --emulate  # standalone emulator
//
...
struct AgentMicroBridgeStatus: Equatable {
...
struct AgentMicroLayout: Equatable {
    ...
    static let defaults = AgentMicroLayout(
        analogStick: [
    ...
    func jsonData() -> Data? {
        ...
        for name in AgentMicroLayout.slotOrder {
        ...
        for dir in AgentMicroLayout.stickOrder {
    ...
    var summary: String {
        let keys = AgentMicroLayout.slotOrder.compactMap { slots[$0]?.keycapId }.joined(separator: " ")
        let stick = AgentMicroLayout.stickOrder
...
final class LayoutWatcher {
    ...
    var onUpdate: (AgentMicroLayout) -> Void = { _ in }

    private let path: String
    private var fileSource: DispatchSourceFileSystemObject?
    private var dirSource: DispatchSourceFileSystemObject?
    private var debounce: DispatchWorkItem?
    private(set) var current = AgentMicroLayout.defaults
    ...
    func start() {
        current = AgentMicroLayout.load(from: path)
    ...
    private func scheduleReparse() {
        ...
        let item = DispatchWorkItem { [weak self] in
            ...
            let parsed = AgentMicroLayout.load(from: self.path)
...
final class Bridge: NSObject, CBCentralManagerDelegate, CBPeripheralDelegate {
    ...
    var onStatusChange: (AgentMicroBridgeStatus) -> Void = { _ in }
    ...
    /// from AgentMicro status without a ChatGPT host present.
    ...
    private func publishStatus() {
        let snapshot = AgentMicroBridgeStatus(
    ...
    /// ChatGPT cache to replay, so re-emit from AgentMicro status instead.
    ...
    /// Force the patched ChatGPT app to re-emit its AgentMicro lighting.
    ...
    func sendLayout(_ layout: AgentMicroLayout) {
...
// MARK: - Built-in virtual AgentMicro (--emulate)
...
final class EmuDevice {
    ...
    /// Device -> host AgentMicro control message on channel 4 (not part of the
    /// AgentMicro protocol). The shim consumes it to run host-side actions like
    ...
    func sendControl(_ obj: [String: Any]) {
        ...
        while offset < payload.count {
            ...
            var report = Data(count: 64)
            report[0] = hidReportID
            report[1] = 4 // AgentMicro control channel (1 debug, 2 RPC, 3 layout)
            report[2] = UInt8(chunk)
...
func startStdinLoop(_ emu: EmuDevice) {
    ...
    Emulate mode - inject device events:
      ...
      enc cw|cc|press
      clear                       clear the ChatGPT composer (AgentMicro channel 4)
      json <raw json>
    """)
...
final class StatusLights {
    ...
    /// always off, regardless of unrelated AgentMicro strip slot numbers.
    ...
    /// Kept as a helper instead of relying on AgentMicro's numeric strip slot:
...
if target == .auto, !emulate {
    ...
    // independent: VS Code re-emits AgentMicro lights, T3 republishes its backend
