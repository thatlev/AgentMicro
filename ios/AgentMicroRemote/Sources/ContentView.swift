...
/// preserves the physical AgentMicro recreation; maximized removes the
...
struct ContentView: View {
    @EnvironmentObject private var peripheral: AgentMicroPeripheral
...
private struct HardwareConsole: View {
    ...
    private func light(at index: Int) -> SlotLight {
        ...
        if selectedIndex == index,
           ...
           !AgentMicroPeripheral.isRecordingGreen(peripheral.ambient.color) {
...
/// the plate, matching the physical AgentMicro caps instead of a flat dot.
...
/// Visual + accessibility description for one AgentMicro keycap legend.
...
/// docs/agent-micro-protocol.md §4. Unknown ids (e.g. custom EMPT caps) fall
...
private struct MicrophoneKey: View {
    /// Sea-green while recording per the AgentMicro voice-capture lighting.
...
private struct DeviceDetailsSheet: View {
    ...
    private func connectionLimitation(for issue: String) -> String {
        if peripheral.managerState == .unsupported {
            return "The iOS Simulator has no Bluetooth peripheral mode. On a physical iPhone, public CoreBluetooth also rejects the mandatory HID Report Reference descriptors that macOS needs to recognize a AgentMicro, so a normal iOS build cannot complete this pairing path."
        }
        ...
        if normalized.contains("hid")
            ...
            return "Public iOS CoreBluetooth cannot publish the complete HID profile macOS needs to recognize a AgentMicro. This build cannot complete the ChatGPT pairing flow; use supported HID hardware or an Apple-approved accessory path."
        }
    ...
    var body: some View {
        List {
            ...
            Section("Synced key bindings") {
                if page == .codex {
                    ForEach(AgentMicroLayout.slotOrder, id: \.self) { slotId in
                        let binding = peripheral.layout.binding(forSlot: slotId)
                        let descriptor = KeycapCatalog.descriptor(for: binding.keycapId)
                        LabeledContent {
...
#Preview {
    ContentView()
        .environmentObject(AgentMicroPeripheral())
}
