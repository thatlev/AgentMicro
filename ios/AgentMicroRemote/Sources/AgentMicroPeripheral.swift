...
//
//  Reverse-engineered protocol notes: ../../../docs/agent-micro-protocol.md
//
...
/// Codex desktop app drives during dictation (docs/agent-micro-protocol.md
...
struct AgentMicroLayout: Equatable {
    ...
    func binding(forSlot slot: String) -> KeyBinding {
        slots[slot] ?? AgentMicroLayout.defaults.slots[slot] ?? KeyBinding(keycapId: slot, commandId: nil)
    }

    /// The action for a stick direction, defaulting like the desktop app.
    func action(forDirection direction: String) -> StickAction? {
        analogStick[direction] ?? AgentMicroLayout.defaults.analogStick[direction]
    }
}
...
@MainActor
final class AgentMicroPeripheral: NSObject, ObservableObject {
    ...
    @Published private(set) var layout: AgentMicroLayout = .defaults
    @Published var focusedApp: String?
    /// Live VS Code state is sent by AgentMicroBridge on private channel 3.
    ...
    /// Clear the ChatGPT composer. The AgentMicro protocol has no composer-clear
    /// command, so this rides a AgentMicro-only control channel (4) that the Mac
    /// shim consumes to run a host-side action; it is never forwarded to ChatGPT's
    /// device layer. See docs/agent-micro-protocol.md §7.
    func clearComposer() {
        sendControl(["cmd": "clearComposer"])
    }

    /// Inserts dictated/typed text straight into ChatGPT's composer. Like
    /// `clearComposer`, the AgentMicro protocol has no insert command, so this
    /// rides the AgentMicro-only shim control channel (4): the Mac shim runs
    ...
    /// `t3code://` handler. Requires the AgentMicroBridge helper to be running;
    ...
    /// cached agent status; the VSCode bridge re-emits AgentMicro status), so the
    ...
    /// Send a AgentMicro-only control message to the Mac shim on channel 4 (not
    /// part of the AgentMicro wire protocol).
    ...
    /// Channel 5 terminates in AgentMicroBridge and never reaches ChatGPT. It
    ...
    private func applyLayout(_ object: [String: Any]) {
        var next = AgentMicroLayout.defaults
        if let slots = object["slots"] as? [String: [String: Any]] {
            for (name, entry) in slots where AgentMicroLayout.slotOrder.contains(name) {
        ...
        if let stick = object["analogStick"] as? [String: [String: Any]] {
            for (dir, entry) in stick where AgentMicroLayout.stickOrder.contains(dir) {
...
private extension ZoneLight {
    init(_ dict: [String: Any]) {
        ...
        else if let e = effectValue as? String { effect = AgentMicroPeripheral.effectId(forName: e) }
...
extension AgentMicroPeripheral: CBPeripheralManagerDelegate {
        ...
        Task { @MainActor in
            if isInputReportCharacteristic(characteristic) {
                guard central.maximumUpdateValueLength >= 63 else {
                    blockingIssue = "The connected host's BLE packet size is too small for 63-byte AgentMicro HID reports."
