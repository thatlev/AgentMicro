# Codex Micro: behavior, protocol, and compatibility research
...
- **[THIRD-PARTY—PHYSICALLY VALIDATED]** Demonstrated on an M5Stack Core2 by the independent
  `imliubo/codex-micro-4-core2` project. It is strong compatibility evidence, but not a dump of
...
| Make a normal iPhone app impersonate Codex Micro directly over Bluetooth | **No. Public iOS CoreBluetooth cannot publish the complete HID-over-GATT profile required by macOS.** | **High** |
...
The viable hardware path is a real Codex Micro or an external BLE-HID peripheral such as an
...
**[OFFICIAL]** Codex Micro is the `kbd-1.0-codex-micro`, a limited-run OpenAI/Work Louder
...
| Dial press | Open/select the focused composer control | Press/release; hold 500 ms opens Codex Micro settings | `ENC`, `act=1/0` | **Official behavior; local ID** |
...
Lighting brightness is configurable from 0–100 in the ChatGPT/Codex Micro settings, defaults to
...
layers whenever iOS returns to the foreground, and asks `CodexMicroBridge` to replay its latest
...
- a native `hid-topology-watcher.node` addon exposes `findCodexMicroInterfaces()` and watches
  topology changes;
...
The active Codex Micro service does **not** call the generic SDK's
...
has not been captured byte-for-byte from a shipping Codex Micro. Labeling it “the confirmed real
...
### Methods used by the current Codex Micro runtime
...
path, not this HID transport. The current Codex Micro service does not invoke most of the generic
...
[`2ee23a4`](https://github.com/imliubo/codex-micro-4-core2/tree/2ee23a4ab696f94bb78d250f28cc4a9b879ba079)
...
Either blocker is fatal to direct iPhone → macOS Codex Micro emulation. Removing `0x2908` may
...
Verified end-to-end: app detects the emulated device, runs the "Welcome to Codex Micro"
...
| Manufacturer matching is part of the active service rule | That filter is in generic SDK code; the active Codex Micro JS service bypasses it. |
...
- `ios/CodexMicroRemote/Sources/CodexMicroPeripheral.swift`
- `ios/CodexMicroRemote/Sources/ContentView.swift`
- `tools/CodexMicroEmu/main.swift`
- `tools/CodexMicroVHid/main.m`
- `tools/CodexMicroBridge/main.swift` (BLE-to-socket relay + `--emulate`)
- `tools/CodexMicroBridge/codex-hid-shim.js` and `tools/patch-chatgpt.sh` (in-app shim route)
...
warning agree with this document. `tools/CodexMicroEmu` is retained only as a GATT reference:
...
- [OpenAI Codex Micro manual](https://learn.chatgpt.com/docs/features/codex-micro)
...
- [M5Stack Core2 Codex Micro emulator, validated commit](https://github.com/imliubo/codex-micro-4-core2/tree/2ee23a4ab696f94bb78d250f28cc4a9b879ba079)
...
- current main-process `codex-micro-service-*.js` and webview chunks
  `codex-micro-bridge`, `codex-micro-layout`, `codex-micro-slot-signals`, and
